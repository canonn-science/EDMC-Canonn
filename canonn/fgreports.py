...
class fgReports(threading.Thread):
    '''
    ...
    def run(self):
        ...
        if not fgReports.bttypes:
            r=requests.get("https://api.canonn.tech:2053/fgtypes")  
            if r.status_code == requests.codes.ok:
                for exc in r.json():
                    if exc.get("journalID"):
                        fgReports.bttypes.append(exc.get("journalID"))
                        fgReports.btlist[exc.get("journalID")]=exc.get("type")
                print fgReports.bttypes
                
                
                
        if self.entry.get("EntryID") in fgReports.bttypes:
            name=fgReports.btlist.get(self.entry.get("EntryID"))
            payload={}
            payload["userType"]="pc"
            ...
            r=requests.post("https://api.canonn.tech:2053/fgReports",data=json.dumps(payload),headers={"content-type":"application/json"})  
